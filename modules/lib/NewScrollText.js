...
function NewScrollText(elementId,dsiplayId,lineHeight,diffHeight,initScrollTop,scrollLine,marqueeDelay) {
	this.elementId = elementId; // 数据列表
	this.dsiplayId = dsiplayId; // 显示列表
	this.lineHeight = lineHeight ? lineHeight : 20; // 行高
	this.marqueeContent = new Array(); // 循环列表
	this.marqueeInterval = new Array(); // 循环句柄
	this.marqueeId = 0; // 循环到第几行了
	this.marqueeDelay = marqueeDelay ? marqueeDelay : 3500;//停顿几秒
	this.scrollLine = scrollLine ? scrollLine : 2;//显示几行
	this.marqueeHeight= this.scrollLine*this.lineHeight+(diffHeight?diffHeight:0); // 跑马灯
...
NewScrollText.prototype.startMarquee = function() {
	var str=$(this.marqueeContent[this.marqueeId]).html(); // 获取 1
